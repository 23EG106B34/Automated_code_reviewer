...
import { analyzeDeveloperEmotion, suggestReviewers } from '../services/emotionAnalyzer.js';
import { reviewCode } from '../services/codeReviewer.js';
import type { ApiResponse, EmotionAnalyzeResponse, TeamMember } from '../types/emotion.js';
import type { CodeReviewResult } from '../services/codeReviewer.js';

export interface FullReviewResponse {
  codeReview: CodeReviewResult;
  emotion: EmotionAnalyzeResponse;
  /** Plain tip tying code issues to how gentle the feedback should be */
  deliveryTip: string;
}
...
/** POST /api/review/check — paste code, get simple fixes */
...
/**
 * POST /api/review/full — code feedback first, then emotion (one click).
 * More code issues → slightly more stressed signals for realistic emotion output.
 */
...
/** GET /api/review/sample — buggy sample for first-time users */
